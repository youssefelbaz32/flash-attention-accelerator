...
#   vivado -mode batch -source fpga/build_bd.tcl -tclargs 8 16 16 16
#                                                          ^BLK ^N ^D ^DV
#
...
foreach m {dma/M_AXI_MM2S dma/M_AXI_S2MM} {
  apply_bd_automation -rule xilinx.com:bd_rule:axi4 \
    -config [list Clk_master {Auto} Clk_slave {Auto} Clk_xbar {Auto} \
                  Master [list /$m] Slave {/ps/S_AXI_HP0_FPD} \
                  intc_ip {Auto} master_apm {0}] \
    [get_bd_intf_pins $m]
}
...
foreach seg [get_bd_addr_segs -excluded -quiet] { }
puts $fh [report_bd_address -return_string]
close $fh
...
set luts [get_property USED [get_cells -hier -filter {PRIMITIVE_GROUP == LUT}] -quiet]
puts "=== $tag ==="
puts "  WNS  : $wns ns"
